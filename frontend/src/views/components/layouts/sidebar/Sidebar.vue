...
<template>
  <aside :class="[{ 'w-60': showMenu }, { 'w-0 md:w-auto': !showMenu }]" class="md:block transition-all bg-white">
    <div class="flex flex-col pb-8 h-full">
      <UserInfo :showMenu="showMenu" />
      <ul class="space-y-2 p-4 font-medium h-full overflow-y-auto">
        <component :is="item.subLinks?SideBarSubLinks:SidebarLink" v-for="item in menu[1]" :key="item.linkName"
          :showMenu="showMenu" :link="item" />
      </ul>

      <button @click="logout" style="background-color: #09244b08"
